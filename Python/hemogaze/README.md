...
# 2. OPTIONAL: fabricate a stand-in dataset (run everything from Python/hemogaze/)
python training/make_synthetic_data.py --out Datasets/hemogaze_sintetico
#    then use training/configs/synthetic.yaml in place of cpanemic.yaml below

# 3. get the real data (see data/README.md), then adapt it to the schema
python processing/prepare_cpanemic.py --src "path/to/CP-AnemiC dataset"

# 4. EDA, confounder audit, and baselines (run and read BEFORE any neural net)
python training/eda_baseline.py --config training/configs/cpanemic.yaml

# 5. train
python training/train.py --config training/configs/cpanemic.yaml            # patient-level
python training/train.py --config training/configs/cpanemic.yaml --all-sites  # cross-site

# 6. figures per run, then the comparison table
python training/evaluate.py --run reports/runs/convnext128_patient_level
python training/evaluate.py --compare          # -> reports/summary.md
```
...
```
processing/     features · splits · metrics · baselines · config   (no torch)
                prepare_cpanemic.py · prepare_external.py
training/       dataset · model · eda_baseline · train · evaluate
                external_validation · make_synthetic_data · configs/
server/         FastAPI service          examples/  web + CLI clients
tests/          54 tests (splits, leakage, metrics, baselines, ROI masking,
                shortcut controls, Hb regression, the serving contract)
Dockerfile      development image; the build fails if the tests fail
reports/        DVC-managed, on DagsHub
